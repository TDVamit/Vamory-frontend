...
// --- AddFromGDriveModal ---
import { useState as useStateGDrive, useEffect as useEffectGDrive } from 'react';
import { X as XGDrive, FolderPlus as FolderPlusGDrive, Link as LinkIcon } from 'lucide-react';
import { filesAPI } from '../services/api';
import { useAuth as useAuthGDrive } from '../hooks/useAuth';
import type { AddFromGDriveRequest } from '../types';
import { UserRole as UserRoleGDrive } from '../types';
...
export const AddFromGDriveModal = ({ isOpen, onClose, onSuccess }: AddFromGDriveModalProps) => {
  const { user } = useAuthGDrive();
  const canAddFromGDrive = user?.user_role === UserRoleGDrive.super_admin || user?.user_role === UserRoleGDrive.admin || user?.user_role === UserRoleGDrive.user;
  if (!isOpen || !canAddFromGDrive) return null;

  const [formData, setFormData] = useStateGDrive<AddFromGDriveRequest>({
    gdrive_url: '',
    folder_name: '',
    folder_storage_type: 'STANDARD_IA',
  });
  const [errors, setErrors] = useStateGDrive<Record<string, string>>({});
  const [isLoading, setIsLoading] = useStateGDrive(false);
  const [error, setError] = useStateGDrive<string | null>(null);
